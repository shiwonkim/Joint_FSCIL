...
from models.archs.networks import FEAT_RECTNet
from utils.feat_rect_logger import LOGGER
from utils.metrics import get_gacc
from utils.feat_rect_tools import get_MVNs

log = LOGGER.LOGGER


class FEAT_RECT(Trainer):
    def __init__(self, args):
        ...
        self.model = FEAT_RECTNet(self.args, mode=self.args.base_mode)
    ...
    def get_dataloader(self, session):
        if session == 0:
            trainset, trainloader, valloader, testloader, trainloader_pk = get_base_dataloader(self.args)
        else:
    ...
    def update_param(self, model, pretrained_dict):
        ...
        chkpt = torch.load(self.args.stage0_chkpt, 'cpu')['backbone_fc']
        chkpt = {i.replace('backbone.', ''): v for i, v in chkpt.items()}
        err = model.module.encoder.load_state_dict(chkpt, strict=False)
        log.info(err)
        
        chkpt = torch.load(self.args.stage0_chkpt, 'cpu')['backbone_fc']  # now cosine classifier only
        chkpt_cls_weight = chkpt['classifier._weights.0'][:self.args.base_class, :]
        for i in model.module.fc.keys():
            model.module.fc[i].weight.data[:len(chkpt_cls_weight), :].copy_(chkpt_cls_weight)
    ...
    def train(self):
        ...
        self.result_list = calc_gflops(self.result_list, self.model.module.encoder, (3, 32, 32))
        self.model = self.update_param(self.model, self.best_model_dict)

        if args.start_session > 0:  # need to test
            train_set, trainloader, valloader, testloader, trainloader_pk = self.get_dataloader(0)  # get session 0 dataloaders

            #_, _, _ = test(self.model, testloader, args, 0, is_feat_rect=True)
            #trainloader.dataset.transform = testloader.dataset.transform # 이부분 검토
            features, features_all, labels = self.model.module.extract_features(self.model.module.encoder, trainloader, args)
            ...
            if args.resume_fr is not None:
                ...
                if args.base_fr:
                    ...
                    train_feature_rectification_base(self.model.module,
                                                     ...
                                                     valloader,
                                                     hyper_params,
                                                     validating_interval=10,
                                                     session=0, )
                    acc_dict, all_acc_per_task, all_acc_per_task_list = test_rectification(self.model.module.encoder,
                                                                                 self.model.module.feature_rectification,
                    ...
                    self.log_session_info(0, acc_dict, g_acc, area)
                    
                    #log.info("acc_dict:" ,acc_dict, "g_acc: ", g_acc, "area:", area)
        ...
        for session in range(start_session, args.sessions):
            ...
            train_set, trainloader, _, testloader, _ = self.get_dataloader(session)
            ...
            if session == 0:
                ...
                _, _, _ = test(self.model, testloader, args, session, is_feat_rect=True)
                ...
                g_acc, area = get_gacc(ratio=int(args.base_class / args.way),
                                       all_acc=all_acc)
                self.log_session_info(session, acc_dict, g_acc, area)
                #log.info("acc_dict:" ,acc_dict, "g_acc: ", g_acc, "area:", area)
    ...
    def set_save_path(self):
        ...
        self.args.save_path = self.args.save_path + self.args.project
