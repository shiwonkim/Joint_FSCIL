...
class CIFAR10(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False,
                 ...
                 extra_data_path=None, samples_per_class=None, img_size=32, is_feat_rect=False,
                 savc_transform=None):
        ...
        if self.train:
            downloaded_list = self.train_list
            if transform is not None: # args.data_aug
        ...
        if base_sess:
            ...
            self.data, self.targets = self.SelectfromDefaultSplit(self.data, self.targets, index)
        ...
        if type(self.targets) == tuple:
            self.targets, self.extra_data = self.targets
        self._load_meta()
    ...
    def SelectfromDefault(self, data, targets, index):
        ...
        for i in index:
            ...
            if (isinstance(data_tmp, list) and len(data_tmp) == 0): # data_tmp == []
    ...
    def SelectfromDefaultSplit(self, data, targets, index):
        data_tmp = []
        targets_tmp = []
        ...
        for i in index:
            ...
            if (isinstance(data_tmp, list) and len(data_tmp) == 0):
                data_tmp = data[ind]
                targets_tmp = targets[ind]
            else:
                data_tmp = np.vstack((data_tmp, data[ind]))
                targets_tmp = np.hstack((targets_tmp, targets[ind]))
...
class CIFAR100Joint(CIFAR100):
    """
    This is a subclass of the `CIFAR100` Dataset customized for joint learning.
    """
    def NewClassSelector(self, data, targets, index):
        data_tmp = []
        targets_tmp = []
        ...
        if self.samples_per_class is not None:
            samples_per_class_incr = len(data_tmp) / len(np.unique(targets_tmp))
            repeat_time = (self.samples_per_class - samples_per_class_incr) // samples_per_class_incr
            data_tmp = np.repeat(data_tmp, repeat_time, axis=0)
            targets_tmp = np.repeat(targets_tmp, repeat_time, axis=0)

        # DeepSMOTE
        if self.extra_data_path is not None:
            extra_datas = np.load(os.path.join(self.extra_data_path, 'images.npy'))
            extra_targets = np.load(os.path.join(self.extra_data_path, 'labels.npy'))
            data_len = len(targets_tmp)
            incr_index = np.unique(targets_tmp)
            data_index = np.where(np.isin(extra_targets, incr_index))[0]
            data_tmp = np.vstack((data_tmp, np.transpose(extra_datas[data_index], axes=(0, 2, 3, 1))))
            targets_tmp = np.hstack((targets_tmp, extra_targets[data_index]))

            extra_or_not = np.ones_like(targets_tmp, dtype=np.bool_)
            extra_or_not[:data_len] = False
            return data_tmp, (targets_tmp, extra_or_not)
