...
class iCIFAR10(DataHandler):
    ...
    train_transforms = [
        transforms.RandomResizedCrop(32), #36에서 수정
        transforms.RandomHorizontalFlip(),
        transforms.ColorJitter(brightness=63 / 255)
    ]
    test_transforms = [
        transforms.Resize(int(32)),
    ...
    def filter_samples_for_fewshot(self, idx_file_path):
        if os.path.exists(idx_file_path) and os.path.isfile(idx_file_path):
            with open(idx_file_path, 'r') as f:
                all_idx = f.read().splitlines()
            new_data, new_target = [], []
            for idx, (data, target) in enumerate(zip(new_data, new_target)):
                if idx in all_idx:
                    new_data.append(data)
                    new_target.append(target)
            self.base_dataset.data = new_data
            self.base_dataset.target = new_target
        else:
            print(f'Open file {idx_file_path} failed!')
...
class CUB200(DataHandler):
    ...
    def base_dataset(self, data_path, train=True, download=False):
        if download:
            warnings.warn(
                "CUB200 incremental dataset cannot download itself,"
                " please see the instructions in the README."
            )

        split = "train" if train else "val"

        print("Loading metadata of CUB_{} ({} split).".format(self.nb_classes, split))
        metadata_path = os.path.join(
            data_path if self.metadata_path is None else self.metadata_path,
            "{}_cub{}{}.txt".format(split, self.nb_classes, self.suffix)
        )
...
class miniimagenet(DataHandler):
    #image_size = 96
    image_size = 84
    train_transforms = [
    ...
    def base_dataset(self, data_path, train=True, download=False):
        if download:
            warnings.warn(
                "miniimagenet incremental dataset cannot download itself,"
                " please see the instructions in the README."
            )

        split = "train" if train else "val"

        print("Loading metadata of miniimagenet_{} ({} split).".format(self.nb_classes, split))
        metadata_path = os.path.join(
            data_path if self.metadata_path is None else self.metadata_path,
            "{}_miniimagenet{}{}.txt".format(split, self.nb_classes, self.suffix)
        )
...
class miniimagenet224(miniimagenet):
    train_transforms = [
        transforms.Resize(256),
        transforms.RandomResizedCrop(224),
        transforms.RandomHorizontalFlip(),
