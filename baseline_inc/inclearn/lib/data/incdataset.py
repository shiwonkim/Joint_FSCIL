...
from .datasets import (
    ...
    iCIFAR100, miniimagenet,
)
import inclearn.lib.utils as utils
from inclearn.lib.data.samplers import RASampler, MyRASampler
...
class IncrementalDataset:
    ...
    def new_task(self, memory=None, memory_val=None, distributed=True, repeated_aug=True, trans_args=None,
                 repeat_split=False):
        ...
        if memory is not None and memory[0] is not None:
        ...
        if distributed:
            ...
            if repeated_aug:
                sampler_train = RASampler(
                    y_train, num_replicas=num_tasks, rank=global_rank, shuffle=True,
                    # length_factor=self._train_loader_len_factor if self._current_task == 0 else 1
                )
            elif repeat_split:
                logger.info(f'using my ra sampler @ 4!!')
                sampler_train = MyRASampler(
                    y_train, num_replicas=num_tasks, rank=global_rank, shuffle=True, num_repeats=4,
                )
            else:
                logger.info(f'using ddp sampler!!')
                sampler_train = torch.utils.data.DistributedSampler(
                    y_train, num_replicas=num_tasks, rank=global_rank, shuffle=True
                )
            sampler_val = torch.utils.data.SequentialSampler(y_test)
            sampler_ddp_type = True
        else:
            sampler_train = None
            sampler_val = None
            sampler_ddp_type = False

        train_loader = self._get_loader(x_train, y_train, train_memory_flags, mode="train", sampler=sampler_train,
                                        sampler_ddp_type=sampler_ddp_type, transform_args=trans_args)
        val_loader = self._get_loader(x_val, y_val, val_memory_flags,
                                      mode="train", transform_args=trans_args) if len(x_val) > 0 else None
        test_loader = self._get_loader(x_test, y_test, np.zeros((x_test.shape[0],)), mode="test", sampler=sampler_val,
                                       sampler_ddp_type=sampler_ddp_type, transform_args=trans_args)
    ...
    def get_custom_loader(
            self, class_indexes, memory=None, mode="test", data_source="train", sampler=None, distributed=False,
            transform_args=None, force_transform=None, force_batchsize=None, current_task_class=False
    ):
        ...
        data, targets = [], []
        if current_task_class:
            min_class = sum(self.increments[:self._current_task - 1])  # self._current_task is added one at new_task()
            max_class = sum(self.increments[:self._current_task])
            x, y = self._select(self.data_train, self.targets_train, low_range=min_class, high_range=max_class)
            data.append(x)
            targets.append(y)
        else:
        ...
        if distributed and (mode == 'test' or mode == 'flip'):
            # todo: add traning
            num_tasks = utils.get_world_size()
            global_rank = utils.get_rank()
            if len(targets) % num_tasks != 0:
                logger.warning(
                    'Warning: Enabling distributed evaluation with an eval dataset not divisible by process number. '
                    'This will slightly alter validation results as extra duplicate entries are added to achieve '
                    'equal num of samples per-process.')
            sampler_val = torch.utils.data.DistributedSampler(
                targets, num_replicas=num_tasks, rank=global_rank, shuffle=False)
            return data, self._get_loader(
                data, targets, memory_flags, shuffle=False, mode=mode, sampler=sampler_val, sampler_ddp_type=True,
                transform_args=transform_args, force_batchsize=force_batchsize
            )
        return data, self._get_loader(
    ...
    def get_memory_loader(self, data, targets, distributed=False, repeated_aug=False, trans_args=None):
        if distributed:
            ...
            if repeated_aug:
                sampler_train = RASampler(
                    targets, num_replicas=num_tasks, rank=global_rank, shuffle=True
                )
            else:
                sampler_train = torch.utils.data.DistributedSampler(
                    targets, num_replicas=num_tasks, rank=global_rank, shuffle=True
        ...
        return self._get_loader(
            data, targets, np.ones((data.shape[0],)), shuffle=True, mode="train", sampler=sampler_train,
            sampler_ddp_type=sampler_ddp_type, transform_args=trans_args
        )
    ...
    def _get_loader(self, x, y, memory_flags, shuffle=True, mode="train", sampler=None, sampler_ddp_type=False,
                    transform_args=None, force_transform=None, force_batchsize=None):
        if mode == "train":
        ...
        if not sampler_ddp_type:
            ...
            trsf = trsf if force_transform is None else force_transform
            ...
            logger.info(f'current transform: \n {trsf}')
        else:
            if self.aa:
                trsf = build_transform(is_train=mode == 'train', args=transform_args)
                logger.info('using aa')
            if mode == 'flip':
                logger.info('added flip transform')
                trsf.transforms.insert(0, transforms.RandomHorizontalFlip(p=1.))
            logger.info(f'current transform: \n {trsf}')
            batch_size = self._batch_size if force_batchsize is None else force_batchsize
            pin_mem = True
            drop_last = True if mode == "train" else False

            prefetch_factor = 4 if mode == "train" else 2
            if self._workers > 0:
                data_loader = DataLoader(
    ...
    def _setup_data(
        ...
        for dataset in datasets:
            train_set = dataset()
            if isinstance(train_set, iCIFAR10):
                train_set.filter_samples_for_fewshot(idx_file_path=os.path.join(data_path, 'all_idx_cifar.txt'))
            train_dataset = train_set.base_dataset(data_path, train=True, download=True)
            ...
            if utils.dist.is_available() and utils.dist.is_initialized():
                utils.dist.barrier()
