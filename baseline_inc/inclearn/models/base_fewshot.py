...
class VitBaselineFewshot(IncrementalLearner):
    ...
    def __init__(self, args: dict):
        ...
        if self.use_teacher_distillate and 'convnet' in args['convnet']:
            ...
            self.teacher_network = network.BaseNet(args=self.args)
            self.teacher_network = nn.DataParallel(self.teacher_network)
            self.teacher_network = self.teacher_network.cuda()
            ...
            model_dict = self.teacher_network.state_dict()
            chkpt = torch.load(args['teacher_path'], 'cpu')['params']
            #chkpt = {i.replace('backbone.', ''): v for i, v in chkpt.items()}

            missing_keys, unexpected_keys = self.teacher_network.load_state_dict(chkpt, strict=True)

            assert not missing_keys and not unexpected_keys, \
            f" State dict loading failed!\nMissing keys: {missing_keys}\nUnexpected keys: {unexpected_keys}"
    ...
    def _before_task(self, train_loader, val_loader, dataset):
        ...
        if self.ddp:
            ...
            self.model_with_ddp = self._network
            self.model_without_ddp = self._network
        all_param = 0
        trainable_param = 0
